...
export default function PortalAccessGate({
  ...
  return (
    <div className="relative min-h-screen">
      ...
      <div className="fixed inset-0 z-40 flex items-center justify-center p-4 bg-gradient-to-br from-gray-900/60 via-purple-900/40 to-purple-900/60 backdrop-blur-sm overflow-y-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-2xl my-8">
          <Card className="p-8 text-center border-2 border-purple-500/40 shadow-2xl bg-background">
            <div className="w-20 h-20 mx-auto rounded-full bg-gradient-to-br from-purple-500 to-purple-700 flex items-center justify-center mb-4">
            ...
            {!pending && (
              <>
                ...
                <Button asChild size="lg" className="bg-gradient-to-r from-purple-600 to-purple-700 hover:opacity-90">
...
function PlanPreview({ name, price, period, recommended = false }: { name: string; price: string; period: string; recommended?: boolean }) {
  return (
    <div className={`rounded-lg border p-3 ${recommended ? "border-purple-500 bg-purple-50/40 dark:bg-purple-950/20" : ""}`}>
      <div className="flex items-center gap-1 text-xs">
        {recommended && <Crown className="w-3 h-3 text-purple-600" />}
